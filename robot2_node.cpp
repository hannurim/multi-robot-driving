...
// Relative coordinates of Robot2 published by Master Robot
{
    Theta12 = atan2(msg->y, msg->x);
}
...
void Identification()
// Index which is Master Robot and Robot3
{
...
// Driving
{
    Theta21 = atan2(r1.goal.second, r1.goal.first);
    double diff = Theta21 - (Theta12 + PI);
    goal.first = r1.goal.first + cos(diff) * Tx12 - sin(diff) * Ty12;
    goal.second = r1.goal.second + sin(diff) * Tx12 + cos(diff) * Ty12;
...
{
    ros::init(argc, argv, "robot2_node");
    ros::NodeHandle nh;

    ROS_INFO("robot2_node start");

    ros::Subscriber sub1 = nh.subscribe("Robot2/scan", 10, sensorCallback);
    ros::Subscriber sub2 = nh.subscribe("Robot12Point", 10, robot1Callback);
    ros::Publisher pub1 = nh.advertise<geometry_msgs::Twist>("Robot2/cmd_vel", 10);
    ros::Publisher pub2 = nh.advertise<geometry_msgs::Point>("Robot21Point", 10);
    ros::Publisher pub3 = nh.advertise<geometry_msgs::Point>("Robot2/TargetPosition", 10);

    geometry_msgs::Point R21;
    geometry_msgs::Point R2_target;

    ros::Rate rate(5);

    while (ros::ok()) {
        R21.x = r1.goal.first;
        R21.y = r1.goal.second;
        R2_target.x = goal.first;
        R2_target.y = goal.second;
        pub1.publish(velOutput);
        pub2.publish(R21);
        pub3.publish(R2_target);
        ros::spinOnce();
        rate.sleep();
    }
